...
本仓库提供 PhaneSeek 桌面版（非官方社区构建）的安装程序与使用文档。Electron 壳源码与构建配置位于 `package-app/`（内置运行时体积较大不入库，构建前按 `package-app/README.md` 说明本地准备）。

> **品牌更名**：自 **1.6.0** 起统一更名为 **PhaneSeek**。应用 `appId`、仓库、安装包名、用户数据目录均已更名，因此旧版本不会自动升级，需手动安装新版安装包。

## 特性

- **一键安装**：NSIS 安装向导，自动创建桌面快捷方式与开始菜单项，图标为 DeepSeek 鲸鱼 logo（多尺寸 16~256px 内置）
- **无边框玻璃窗口**：顶部品牌胶囊（PhaneSeek · 法涅斯）+ 右上角悬浮胶囊按钮组（最小化 / 最大化 / 关闭），明暗自适应
...
- **内置最新运行时**：随包 dsh 框架与官方 WebUI 界面均为 `0.1.5-rc.2`（v1.6.0）
...
- **全新配置**：不迁移任何旧版数据（含旧 `%USERPROFILE%\.dsh`），首次启动即为全新配置（v1.6.0）
...
**最新版本：v1.6.0**（[更新说明](https://github.com/FeatherCloudSky/PhaneSeek/releases/tag/ver1.6.0)）

前往 [Releases](https://github.com/FeatherCloudSky/PhaneSeek/releases) 下载最新版 `PhaneSeek-Setup-1.6.0.exe`。
...
1. 双击 `PhaneSeek-Setup-1.6.0.exe`，按向导操作；
2. 可自定义安装目录（默认 `%LOCALAPPDATA%\Programs\PhaneSeek`）；
3. 向导自动创建桌面快捷方式与开始菜单项；
4. 安装完成后自动启动应用。

> 提示：安装前请先关闭正在运行的 PhaneSeek（或旧版本）窗口，避免程序文件被占用导致安装/升级失败。
...
- 首次启动为全新配置：不迁移任何旧版数据（含旧 `%USERPROFILE%\.dsh`）；如需沿用旧会话，请手动把旧数据目录中的会话/配置文件复制到 `%APPDATA%\PhaneSeek\dsh-home`（**切勿复制 `profiles\node_modules`**，dsh 0.1.5 会因此拒绝启动）；
- 关闭窗口 = 停止服务（无后台残留）。

| 项 | 位置 |
|---|---|
| 用户数据（会话/配置/插件） | `%APPDATA%\PhaneSeek\dsh-home` |
| 界面缓存（Chromium） | `%APPDATA%\PhaneSeek\user-data` |
| 服务端口 | `127.0.0.1:8898`（固定） |
| 内置框架 / WebUI 版本 | `0.1.5-rc.2` |
...
- 本项目为**非官方社区构建**，与 DeepSeek 官方无隶属、赞助或背书关系；应用界面品牌标识为「PhaneSeek 法涅斯」；
